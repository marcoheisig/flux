...
void GridItem::precalculateBoundingBox()
{
    if(grid)
    {
        bRect.setWidth((grid->xSize()) * gridSpacing );
        bRect.setHeight((grid->ySize()) * gridSpacing);
        bRect.moveTo(0,0);
    }
    else
        bRect = QRectF();
...
void GridItem::drawVelocityArrows( QPainter * p )
{
   ...
   for ( int j = 0; j < grid->ySize (); ++j )
      for ( int i = 0; i < grid->xSize (); ++i )
      {
         ...
         unsigned int inv_j = (unsigned int)(( grid->ySize () - 1 ) - j);
...
void GridItem::drawArray(QPainter * p,const Array<real> & a, QRectF ellipseRect, const QColor & c)
{
