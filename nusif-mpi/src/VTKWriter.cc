        ...
        writeVelocity_(writeVelocity), writePressure_(writePressure), counter_ (0 )
{
   ...
   sstream << "DIMENSIONS " << grid_.xSize()-2 << " " << grid_.ySize()-2 << " 1\n";
   sstream << "ORIGIN 0 0 0 \n";
   sstream << "SPACING " << grid_.dx() << " " << grid_.dy() << " 1\n";
   sstream << "POINT_DATA " << (grid_.xSize()-2) * (grid_.ySize()-2) << " \n" << std::endl;
...
void VTKWriter::write()
{
   ...
   if ( writeVelocity_ )
   {
      ...
      for ( int j = 0; j < grid_.ySize ()-2; ++j )
         for ( int i = 0; i < grid_.xSize ()-2; ++i )
         {
   ...
   if ( writePressure_ )
   {
      ...
      for ( int j = 1; j < grid_.ySize ()-1; ++j )
         for ( int i = 1; i < grid_.xSize ()-1; ++i )
            fileStream << grid_.p()( i, j ) << "\n";
   }
